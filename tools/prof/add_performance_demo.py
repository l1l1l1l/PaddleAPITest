...
m = 66244
test_loop = 240662
numpy_tensor1 = (numpy.random.random([m]) - 0.5).astype("float32")
...
paddle_out = paddle.Tensor.add(paddle_x1, paddle_x2)

with paddle.no_grad():
    paddle.base.core._cuda_synchronize(paddle.CUDAPlace(0))
    start = time.time()
    for _i in range(test_loop):
        paddle.Tensor.add(paddle_x1, paddle_x2)
    paddle.base.core._cuda_synchronize(paddle.CUDAPlace(0))
    end = time.time()
    timeused = end - start
...
torch_out = torch.Tensor.add(torch_x1, torch_x2)

with torch.no_grad():
    torch.cuda.synchronize()
    start = time.time()
    for _i in range(test_loop):
        torch.Tensor.add(torch_x1, torch_x2)
    torch.cuda.synchronize()
    end = time.time()
    timeused = end - start
