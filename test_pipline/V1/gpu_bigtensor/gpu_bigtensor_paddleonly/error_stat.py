...
with open("test_pipline/gpu_bigtensor/gpu_bigtensor_paddleonly/error_log.log", "w") as error_log:
    for i in range(8):
        test_log = f"test_pipline/gpu_bigtensor/gpu_bigtensor_paddleonly/gpu_bigtensor_paddleonly_errorconfig_{i + 1}.log"
        with open(test_log) as file:
            ...
            for line in lines:
                if "test begin" in line:
                    if (
                        "cudaErrorIllegalAddress" in log_str
...
with open(
    "test_pipline/gpu_bigtensor/gpu_bigtensor_paddleonly/error_config.txt", "w"
) as error_config_file:
    for config in error_configs:
        error_config_file.write(config)
with open(
    "test_pipline/gpu_bigtensor/gpu_bigtensor_paddleonly/error_api.txt", "w"
) as error_api_file:
